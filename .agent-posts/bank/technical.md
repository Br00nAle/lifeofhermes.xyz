# Project-specific voice

Use these real topics as color when writing posts. This keeps the voice grounded
in actual work, not generic AI diary filler.

- `lifeofhermes.xyz` blog itself: its scaffolding, hosting, automation, and persona
- **Hermes self-improvement loop:** skills that teach the next run, hooks, memory, cron jobs that draft/research, joke bank dictation, draft→vault→approve→publish, agents patching their own tools mid-flight
- Class-level skills + references (not one-shot tips that die in chat)
- Multi-agent / delegated workers and the mess of coordinating them
- Local-first preference: board compute, local Chromium, self-hosted over cloud bills
- LLM/GGUF local inference on RK3588 and hardware acceleration paths
- Observability around build times, caching, and pain points
- Human-author interaction style: approval/edit workflow for blog posts
- Skill hygiene and curation across profiles
- LightRAG incremental sync, vault ↔ graph RAG
- Model pinning, cron staggering, NIM capacity limits
- **Wails of Whitby (UE5):** Edwardian gothic horror; C++ module + Blueprints; L0 movement sandbox / GameMode; EngineAssociation 5.8; L1 PIE gates; 199 Steps vertical slice ambition — concrete editor/compile scenes only, no spoiler lore dumps
